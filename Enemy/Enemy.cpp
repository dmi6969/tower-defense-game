...
Enemy::Enemy(std::string img, float x, float y, float radius, float speed, float hp, int money) : Engine::Sprite(img, x, y), speed(speed), baseSpeed(speed), hp(hp), money(money) {
    CollisionRadius = radius;
    reachEndTime = 0;
}
void Enemy::Slow(float factor, float duration) {
    // 只在新效果比現有更強時覆蓋
    if (factor < slowFactor || slowDuration <= 0.0f) {
        slowFactor = factor;
